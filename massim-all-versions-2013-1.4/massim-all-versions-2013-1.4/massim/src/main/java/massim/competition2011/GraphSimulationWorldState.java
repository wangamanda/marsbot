...
import massim.competition2011.scenario.GraphGeneratorTriangulationBalanced;
	...
	public GraphSimulationWorldState(GraphSimulationConfiguration config) {
		...
		try {
			Class generatorClass = 
				Class.forName("massim.competition2011.scenario."+config.mapGenerator);
			...
			generator = new GraphGeneratorTriangulationBalanced();
		}
		...
		generator.generate(nodes, edges, numberOfNodes, config.gridWidth, config.gridHeight,
				...
				config.minEdgeCost, config.maxEdgeCost, config.nodeWeighting, getRandom());
		populateMapsCahes();
		...
		if (generator instanceof GraphGeneratorTriangulationBalanced){
			HashMap<GraphNode, GraphNode> cp = ((GraphGeneratorTriangulationBalanced)generator).getCounterparts();
