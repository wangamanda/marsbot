...
import massim.competition2012.monitor.render.Renderer;
...
/**
 * This is the RMI XML monitor application for 2012 Mars Scenario. Start with these options:<br/>
 * <br/>
 ...
 */
public class GraphMonitor extends JFrame implements ActionListener, Runnable {
	...
	private VisMode visMode = VisMode.NORMAL;
...
//	public boolean monitorStarted = false;
	...
	private class WorldView extends JPanel implements MouseListener, ActionListener {
		...
		public void paint(Graphics g) {
			...
			synchronized (syncObject) {
				...
				// draw the background
				if ( drawBackground ) g2d.drawImage(bgImage, 0, 0, sizeX, sizeY, null);

				// take zoom into account
				g2d.scale(scale, scale);				
				
				// set up renderers... different modes = different sets ofrenderers
				Vector<Renderer> renderers = new Vector<Renderer>();
				if ( visMode == VisMode.POLYGONS ) { 
					renderers.add(Renderer.createRenderer("zones"));
					renderers.add(Renderer.createRenderer("noNumbersTopology"));
					renderers.add(Renderer.createRenderer("noNumbersAgents"));
				}
				else {
					renderers.add(Renderer.createRenderer("edges"));
					renderers.add(Renderer.createRenderer("nodes"));					
				}
			...
			synchronized (syncObject) {
				for (NodeInfo n : nodes) {
					...
					for (AgentInfo ag : n.agents) {
						...
						if (Math.abs(ag.x - x) <= Definitions.agentRadius
								&& Math.abs(ag.y - y) <= Definitions.agentRadius) {
		...
		public void actionPerformed(ActionEvent e) {
			if ("zoomIn".equals(e.getActionCommand())){
				...
				switch ( visMode ) {
				case NORMAL:
					visMode = VisMode.POLYGONS;
					break;
				case POLYGONS:
					visMode = VisMode.NORMAL;
					break;
				default:
					assert false: "unexpected visMode " + visMode;
				}
	...
	private class InfoPanel extends JPanel implements PropertyChangeListener{
		...
		public void update(){
			...
			synchronized (syncObject) {
				...
				comboBox.setSelectedItem(selectedAgent);
			}
	...
	public GraphMonitor(){
		...
		this.setTitle("Agent Contest 2012");
	...
	protected void addButtons(JPanel menuPanel) {
		...
		button.addActionListener(worldView);
		button.setActionCommand("toggleBackground");
		menuPanel.add(button);

		button = new JButton("Mode");
		button.addActionListener(worldView);
		button.setActionCommand("toggleMode");
		menuPanel.add(button);
		
		comboBox = new JComboBox();
		comboBox.addActionListener (worldView);
		comboBox.setActionCommand("selectAgent");
