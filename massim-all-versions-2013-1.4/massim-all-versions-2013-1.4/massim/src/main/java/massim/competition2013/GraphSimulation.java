...
import massim.competition2013.scenario.ActionExecutor;
import massim.competition2013.scenario.DominatedArea;
import massim.competition2013.scenario.DominationCalculator;
...
/**
 * This is the main class for the GraphSimulation (2013 Mars Scenario).
 */
public class GraphSimulation extends AbstractSimulation{
	...
	public void initializeSimpleSimulation() {
		...
		Vector<Integer> agentPositions = new Vector<Integer>(); //used to store the agent's positions to ensure symmetric placement
		for (int i = 0; i < agents.length; i++) {
			GraphSimulationAgent simAgent = (GraphSimulationAgent) agents[i];
			simAgent.initialize(config);
			state.addAgent( (GraphSimulationAgentState)simAgent.getAgentState(), agentPositions,
					 i < agents.length/2);
	...
	public String finalizeSimpleSimulation() {
		...
		if(rankings.firstElement().summedScore + rankings.firstElement().currAchievementPoints == rankings.get(1).summedScore+rankings.get(1).currAchievementPoints){
			return "draw";
		}
